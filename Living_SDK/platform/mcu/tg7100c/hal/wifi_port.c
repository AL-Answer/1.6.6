...
static void __event_cb_wifi_event(input_event_t *event, void *private_data)
{
    ...
    switch (event->code) {
        ...
        {
            os_printf("[APP] [EVT] disconnect , Reason: %d\r\n", event->value);
        }
        break;
...
static int wifi_init(hal_wifi_module_t *m) {
  ...
  memcpy((char *)mac, hfilop_layer_get_mac(), 6);
...
static int wifi_transform_rssi(int rssi_dbm)
{
	int ret;
	ret = (rssi_dbm+95)*2;	
...
static void scan_ap_info_copy(ap_list_adv_t *dst, wifi_mgmr_scan_item_t *src)
{
	memset((char *)dst, 0, sizeof(ap_list_adv_t));
	dst->security=src->auth;
	dst->channel = src->channel;
	dst->ap_power = wifi_transform_rssi(src->rssi);
...
int hfilop_wifi_scan(wifi_scan_callback_t cb)
{
	 ...
	 for(channel=1; channel<=14; channel++)
	 {
		...
		{
			if(wifiMgmr.scan_items[i].is_used && channel==wifiMgmr.scan_items[i].channel) 
			{
				scan_ap_info_copy(&info, &(wifiMgmr.scan_items[i]));
				if(cb != NULL)
					cb(&info);
			}
		}

...
static wifi_event_pkt_cb_t cb_mgmt;
static void* cb_mgmt_env;
...
int bl_rx_mgmt_cb_register(void *env, wifi_event_pkt_cb_t cb)
{
    cb_mgmt = cb;
    cb_mgmt_env = env;
...
//FIXME race condition protect?
int bl_rx_mgmt_cb_unregister(void *env)
{
    cb_mgmt = NULL;
    cb_mgmt_env = NULL;
...
void bl_rx_mgmt_cb(uint8_t *pkt, int len)
{
    if (cb_mgmt) {
        cb_mgmt(cb_mgmt_env, pkt, len);
    }
}

...
//mode = 0, for normal adc. freq 40HZ~900HZ
//mode = 1, for mic, freq 500HZ~24000HZ
int hal_bladc_init(int mode, int freq, int data_num, int gpio_num)
{
    ...
    if (mode == 0) {
        if (freq < 40 || freq > 900) {
            blog_error("illegal freq. for mode0, freq 40HZ ~ 900HZ \r\n");
            
            return -1;
        }
    } else if (mode == 1) {
        if (freq < 500 || freq > 24000) {
            blog_error("illegal freq. for mode1, freq 500HZ ~ 24000HZ \r\n");
    ...
    bl_adc_init(mode, gpio_num, freq);

...
if (ilife_ota_info.target_ver != ilife_getMcuOtaVer(ctx->ota_ver))
        {
            LOG_TRACE("New Target Ver start new download................");
...
mcu_update_state = 1;
    ...
    else if (ctx->upg_mcu_flag == 1)
    {
        //ilife_ota_info.update_status = 0;
        HAL_Printf("ota download over type = %d\n", ota_param->res_type);
    } else {
      HAL_Printf("ota over reboot\n");
        ota_reboot();
    }
    #else
#if ((defined (TG7100CEVB)) || (defined (BOARD_ESP8266)))
